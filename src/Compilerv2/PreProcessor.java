...
public class PreProcessor {
    ...
    private static ArrayList<UserLine> loadFile(String filePath, int realLineOfFilePath) {
        ...
        try {
            ...
            ASMCompiler.PreProcessorError("Cannot read file Specified", realLineOfFilePath);
        }
        return file;
    }
    ...
    private static void addToDefineData(UserLine line) {
        ...
        if (nameAndValue.length != 2) {
            ASMCompiler.PreProcessorError("Cannot add Define too many/little arguments", line.realLineNumber);
        } else {
    ...
    public static ArrayList<UserLine> preProcess(ArrayList<UserLine> file, boolean outputFile) {
        ...
        for (int i = 0; i < file.size(); i++) {
            ...
            if (currentLine.line.startsWith("#include")) {
                ...
                //file.add(i, currentLine);
            }
        }

        if (outputFile) {
            try (PrintWriter out = new PrintWriter("preProcessedFile.asm")) {

                for (int i = 0; i < file.size(); i++) {

                    out.println(file.get(i).line);
                }
